...
let achievementTracking = {
    ...
    arenaWinStreak: 0,
    perfectArenaWins: 0,
    cardsInspected: new Set(),
    modalsOpened: new Set(),
    koinSpentToday: 0,
    stardustEarned: 0,
    stardustSpent: 0,
    cardsRecycledBySR: 0,
    spinPacksPurchased: 0,
    dailyRewardsClaimed: 0,
    speedOpeningSession: 0,
    speedOpeningStartTime: null,
    collectionBookTimeTotal: 0,
    collectionBookOpenTime: null,
    firstPackOfDay: false,
...
function loadState() {
    ...
    if (savedPackInventory) {
        packInventory = JSON.parse(savedPackInventory);
    }
...
function claimDailyReward() {
    ...
    // ===== TRACKING DE LOGROS =====
    achievementTracking.dailyRewardsClaimed++;
    localStorage.setItem('dailysClaimed', parseInt(localStorage.getItem('dailysClaimed') || 0) + 1);
    
    // Track max daily streak
    const maxStreak = parseInt(localStorage.getItem('maxDailyStreak') || 0);
    if (currentDay > maxStreak) {
        localStorage.setItem('maxDailyStreak', currentDay);
    }
...
function redeemCode() {
    ...
    if (REDEEM_CODES_INFO[code]) {
        ...
        achievementTracking.codesRedeemed.add(code);
        if (code === 'YEREMY') {
            localStorage.setItem('yeremeCodeRedeemed', 'true');
        }
        ...
        achievementTracking.failedCodeAttempts++;
        $codeMessage.textContent = 'Código no válido o expirado.';
        $codeMessage.style.color = '#ef4444';
        if (achievementTracking.failedCodeAttempts >= 5) {
            localStorage.setItem('failedCodeAttempts', achievementTracking.failedCodeAttempts);
        }
    }
}
...
function buyShopItem(itemId, quantity = 1) {
    ...
    // Tracking de Koins gastados
    const koinsSpentToday = parseInt(localStorage.getItem('koinsSpentToday') || 0);
    localStorage.setItem('koinsSpentToday', koinsSpentToday + totalCost);
    
    const koinsSpentTotal = parseInt(localStorage.getItem('koinsSpentTotal') || 0);
    localStorage.setItem('koinsSpentTotal', koinsSpentTotal + totalCost);

    if (item.packType) {
        packInventory[item.packType] = (packInventory[item.packType] || 0) + quantity;
        if (item.packType.toLowerCase().includes('spin')) {
            achievementTracking.spinPacksPurchased++;
        }
...
function openLuckySpin() {
    ...
    $openLuckySpinBtn.disabled = true;
    const $luckySpinStatus = document.getElementById('lucky-spin-status');
    if ($luckySpinStatus) {
        $luckySpinStatus.textContent = "Abriendo Lucky Spin...";
    }
    ...
    setTimeout(() => {
        try {
            ...
            const $luckySpinStatus = document.getElementById('lucky-spin-status');
            if ($luckySpinStatus) {
                $luckySpinStatus.textContent = "Lucky Spin (20)";
            }
...
function openPack(chosenType) {
    ...
    // ===== TRACKING DE LOGROS TEMPORALES =====
    const currentTime = new Date();
    const hours = currentTime.getHours();
    
    // Track early morning opens (antes de 8 AM)
    if (hours < 8) {
        localStorage.setItem('earlyPackOpens', parseInt(localStorage.getItem('earlyPackOpens') || 0) + 1);
    }
    
    // Track late night opens (después de 11 PM)
    if (hours >= 23) {
        localStorage.setItem('latePackOpens', parseInt(localStorage.getItem('latePackOpens') || 0) + 1);
    }
    ...
    // Tracking de logros: contar SRs en este sobre
    const srCount = packCards.filter(card => card.rarity === 'SR').length;
    const maxSrInOnePack = parseInt(localStorage.getItem('maxSrinOnePack') || 0);
    if (srCount > maxSrInOnePack) {
        localStorage.setItem('maxSrinOnePack', srCount);
    }
    ...
    setTimeout(() => {
        try {
            ...
            if ($openPackBtn) {
                $openPackBtn.disabled = false;
                $openPackBtn.textContent = "Abrir Sobre";
            }
            updateLuckySpinUI();
...
function showCollectionBook() {
    ...
    // Track modal abierto
    achievementTracking.modalsOpened.add('collection_book');
    achievementTracking.collectionBookOpenTime = Date.now();
...
function hideCollectionBook() {
    // Track time spent in collection book
    if (achievementTracking.collectionBookOpenTime) {
        const timeSpent = Math.floor((Date.now() - achievementTracking.collectionBookOpenTime) / 1000);
        achievementTracking.collectionBookTimeTotal += timeSpent;
        achievementTracking.collectionBookOpenTime = null;
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $openMenuArenaBtn = document.getElementById('menu-arena');
    ...
    // Tracking de días jugados para logros
    ...
    if (lastPlayedDate !== today) {
        ...
        if (!playedDates.includes(today)) {
            ...
            achievementTracking.daysPlayedList = playedDates;
        }
    ...
    $openMenuArenaBtn && $openMenuArenaBtn.addEventListener('click', (e) => {
        e.preventDefault();
        showArenaModal();
    });

    $openSoulMarketBtn && $openSoulMarketBtn.addEventListener('click', (e) => {
        e.preventDefault();
        showSoulMarketModal();
    });
    $closeSoulMarketBtn && $closeSoulMarketBtn.addEventListener('click', hideSoulMarketModal);
    ...
    $closeArenaBtn && $closeArenaBtn.addEventListener('click', hideArenaModal);
    $arenaEndBtn && $arenaEndBtn.addEventListener('click', endArena);
    $arenaAutoPlayBtn && $arenaAutoPlayBtn.addEventListener('click', autoPlayArena);
...
// ========== FUNCIONES HELPER PARA TRACKEAR EVENTOS DE LOGROS ==========
function trackArenaWin() {
    achievementTracking.arenaWins++;
    achievementTracking.arenaWinStreak++;
    localStorage.setItem('arenaWins', achievementTracking.arenaWins);
    checkAchievements();
}

function trackArenaLoss() {
    achievementTracking.arenaWinStreak = 0;
    localStorage.setItem('arenaWinStreak', 0);
}

function trackArenaBattle() {
    achievementTracking.arenaBattles++;
    localStorage.setItem('arenaBattles', achievementTracking.arenaBattles);
    checkAchievements();
}

function trackQuestCompleted() {
    const totalCompleted = parseInt(localStorage.getItem('totalQuestsCompleted') || 0) + 1;
    localStorage.setItem('totalQuestsCompleted', totalCompleted);
    checkAchievements();
}

function trackCardInspected(cardName) {
    achievementTracking.cardsInspected.add(cardName);
    checkAchievements();
}

function trackModalOpened(modalName) {
    achievementTracking.modalsOpened.add(modalName);
    checkAchievements();
}

function trackStardustEarned(amount) {
    achievementTracking.stardustEarned += amount;
    localStorage.setItem('stardustEarned', achievementTracking.stardustEarned);
}

function trackCollectionBookTime(seconds) {
    achievementTracking.collectionBookTimeTotal += seconds;
    if (achievementTracking.collectionBookTimeTotal >= 120) {
        localStorage.setItem('collectionBookTime120s', 'true');
    }
}
...
function checkAchievements() {
    ...
    ACHIEVEMENTS.forEach(achievement => {
        ...
        switch (achievement.requirement.type) {
            ...
            case 'rarity_count':
                const rarityKey = achievement.requirement.rarity;
                currentProgress = RARITIES[rarityKey]?.count || 0;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'set_cards_count':
                const setCards = [];
                const set = MOCK_CARDS_BY_SET[achievement.requirement.set];
                if (set) {
                    for (const rarity in set) {
                        setCards.push(...(set[rarity] || []));
                    }
                }
                currentProgress = setCards.filter(card => collection[card] && collection[card] > 0).length;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'koins_spent':
                currentProgress = parseInt(localStorage.getItem('koinsSpentTotal') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'arena_wins':
                currentProgress = achievementTracking.arenaWins;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'arena_battles':
                currentProgress = achievementTracking.arenaBattles;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'arena_win_streak':
                currentProgress = achievementTracking.arenaWinStreak;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'daily_streak':
                currentProgress = dailyRewardsStatus.currentStreak || 0;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'days_played':
                currentProgress = achievementTracking.daysPlayedList.length;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'codes_redeemed':
                currentProgress = achievementTracking.codesRedeemed.size;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'quests_completed':
                currentProgress = parseInt(localStorage.getItem('totalQuestsCompleted') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'stardust_accumulated':
                currentProgress = stardust;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'sr_in_one_pack':
                // Esto se trackea cuando se abre un pack
                currentProgress = parseInt(localStorage.getItem('maxSrinOnePack') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'all_sets_complete':
                const completeSets = countCompleteSets();
                const totalSets = Object.keys(MOCK_CARDS_BY_SET).length;
                currentProgress = completeSets;
                shouldUnlock = completeSets === totalSets;
                break;
            case 'specific_card':
                shouldUnlock = collection[achievement.requirement.card] && collection[achievement.requirement.card] > 0;
                currentProgress = shouldUnlock ? 1 : 0;
                break;
            case 'specific_cards':
                const hasAllCards = achievement.requirement.cards.every(card => 
                    collection[card] && collection[card] > 0
                );
                shouldUnlock = hasAllCards;
                currentProgress = shouldUnlock ? 1 : 0;
                break;
            case 'unopened_packs':
                currentProgress = Object.values(packInventory).reduce((sum, qty) => sum + qty, 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'max_daily_streak':
                currentProgress = parseInt(localStorage.getItem('maxDailyStreak') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'achievements_unlocked':
                currentProgress = achievementsStatus.unlocked.length;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'first_pack_rarity':
                if (achievementTracking.firstPackOfDay) {
                    currentProgress = 1;
                    shouldUnlock = true;
                }
                break;
            case 'set_sec_complete':
                const setSecs = MOCK_CARDS_BY_SET[achievement.requirement.value]?.SEC || [];
                const secOwned = setSecs.filter(card => collection[card] && collection[card] > 0).length;
                currentProgress = secOwned;
                shouldUnlock = secOwned === setSecs.length && setSecs.length > 0;
                break;
            case 'db_sr_cards':
                const dbCards = [];
                ['DB', 'DBZ', 'DBGT', 'DBS'].forEach(set => {
                    const srCards = MOCK_CARDS_BY_SET[set]?.SR || [];
                    dbCards.push(...srCards);
                });
                currentProgress = dbCards.filter(card => collection[card] && collection[card] > 0).length;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'perfect_arena_win':
                currentProgress = achievementTracking.perfectArenaWins;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'upset_victory':
                currentProgress = parseInt(localStorage.getItem('upsetVictories') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'sec_used_arena':
                currentProgress = parseInt(localStorage.getItem('secUsedArena') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'low_rarity_win':
                currentProgress = parseInt(localStorage.getItem('lowRarityWins') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'consecutive_r_spins':
                currentProgress = achievementTracking.twoRInRowSpins;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'double_reward_spin':
                currentProgress = parseInt(localStorage.getItem('doubleRewardSpins') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'sr_from_spins':
                currentProgress = parseInt(localStorage.getItem('srFromSpins') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'daily_claimed':
                currentProgress = parseInt(localStorage.getItem('dailysClaimed') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'quests_completed_daily':
                currentProgress = parseInt(localStorage.getItem('questsCompletedToday') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'early_pack_open':
                currentProgress = parseInt(localStorage.getItem('earlyPackOpens') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'late_pack_open':
                currentProgress = parseInt(localStorage.getItem('latePackOpens') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'hourly_pack_claimed':
                currentProgress = parseInt(localStorage.getItem('hourlyPacksClaimed') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'surtido_pack_opened':
                currentProgress = parseInt(localStorage.getItem('surtidoPacksOpened') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'set_completion_percent':
                const setName = achievement.requirement.value;
                const allCards = MOCK_CARDS_BY_SET[setName];
                if (allCards) {
                    const totalInSet = Object.values(allCards).reduce((sum, arr) => sum + arr.length, 0);
                    const ownedInSet = Object.values(allCards).reduce((sum, arr) => 
                        sum + arr.filter(card => collection[card] && collection[card] > 0).length, 0
                    );
                    currentProgress = totalInSet > 0 ? Math.round((ownedInSet / totalInSet) * 100) : 0;
                    shouldUnlock = currentProgress >= achievement.requirement.value;
                }
                break;
            case 'ginyu_force_complete':
                const ginyu = ['Ginyu', 'Recoom', 'Burter', 'Jeice', 'Guldo'];
                const ginyuOwned = ginyu.filter(card => collection[card] && collection[card] > 0).length;
                currentProgress = ginyuOwned;
                shouldUnlock = ginyuOwned === ginyu.length;
                break;
            case 'all_pack_types_opened':
                const packTypesOpened = new Set();
                ['DB', 'DBZ', 'DBGT', 'DBS', 'MHA', 'KNY', 'GF', 'GUM', 'HB', 'UT', 'DLH', 'FNAFHS', 'DBD', 'DBH'].forEach(set => {
                    if (Object.values(MOCK_CARDS_BY_SET[set] || {}).some(arr => arr.some(card => collection[card] && collection[card] > 0))) {
                        packTypesOpened.add(set);
                    }
                });
                currentProgress = packTypesOpened.size;
                shouldUnlock = packTypesOpened.size >= 5; // Mínimo 5 animes diferentes
                break;
            case 'villain_cards':
                const villains = ['Frieza', 'Goku Black', 'Vegeta', 'Ginyu', 'Nappa', 'Dabi', 'Toga'];
                const villainsOwned = villains.filter(card => collection[card] && collection[card] > 0).length;
                currentProgress = villainsOwned;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'card_inspections':
                currentProgress = achievementTracking.cardsInspected.size;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'all_modals_opened':
                currentProgress = achievementTracking.modalsOpened.size;
                shouldUnlock = currentProgress >= 6; // Mínimo de modales importantes
                break;
            case 'collection_reset':
                currentProgress = parseInt(localStorage.getItem('collectionResets') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'speed_opening':
                currentProgress = parseInt(localStorage.getItem('speedOpenings') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'collection_book_time':
                currentProgress = achievementTracking.collectionBookTimeTotal;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'spin_pack_purchase':
                currentProgress = achievementTracking.spinPacksPurchased;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'mixed_anime_pack':
                currentProgress = parseInt(localStorage.getItem('mixedAnimePacks') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'failed_code_attempts':
                currentProgress = achievementTracking.failedCodeAttempts;
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'specific_code':
                shouldUnlock = achievementTracking.codesRedeemed.has(achievement.requirement.code);
                currentProgress = shouldUnlock ? 1 : 0;
                break;
            case 'lucky_sec_rate':
                currentProgress = parseInt(localStorage.getItem('secFromLuckySpinCount') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'stardust_converted':
                currentProgress = parseInt(localStorage.getItem('stardustConverted') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'arena_cards_deployed':
                currentProgress = parseInt(localStorage.getItem('arenaCardsDeployed') || 0);
                shouldUnlock = currentProgress >= achievement.requirement.value;
                break;
            case 'duplicate_sr_copies':
                // Encontrar la tarjeta SR con más copias
                let maxCopies = 0;
                for (const [card, rarity] of Object.entries(CARD_TO_RARITY)) {
                    if (rarity === 'SR' && collection[card] && collection[card] > maxCopies) {
                        maxCopies = collection[card];
                    }
                }
                currentProgress = maxCopies;
                shouldUnlock = maxCopies >= achievement.requirement.value;
                break;
            case 'all_sets_cards':
                // Verificar que tengamos al menos una carta de cada set
                const setsWithCards = Object.keys(MOCK_CARDS_BY_SET).filter(set => {
                    const setCards = Object.values(MOCK_CARDS_BY_SET[set]).flat();
                    return setCards.some(card => collection[card] && collection[card] > 0);
                });
                currentProgress = setsWithCards.length;
                shouldUnlock = setsWithCards.length === Object.keys(MOCK_CARDS_BY_SET).length;
                break;
        }
