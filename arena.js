...
function evaluateArenaEnd() {
    ...
    if (playerCount === 0 || enemyCount === 0) {
        ...
        // ===== TRACKING DE LOGROS DE ARENA =====
        if (enemyCount === 0) {
            // Victoria: incrementar contador de victorias
            ...
            localStorage.setItem('arenaWinStreak', achievementTracking.arenaWinStreak);
            
            // Verificar si fue una victoria perfecta (sin perder cartas)
            if (playerCount > 0 && playerCount === getArenaTeamCount('player')) {
                achievementTracking.perfectArenaWins++;
                localStorage.setItem('perfectArenaWins', achievementTracking.perfectArenaWins);
            }
